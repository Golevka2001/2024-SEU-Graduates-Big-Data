...
from stat_data.models import GraduatePersonalStat, BirthDateStat, OriginStat, FavoriteCanteenStat, LibraryBorrowingStat
...
def stat_data_view(request):
    ...
    borrowing_rank = GraduatePersonalStat.objects.filter(total_borrowed_books_num__gt=graduate.total_borrowed_books_num).count()
    ...
    top_borrower_in_unit = LibraryBorrowingStat.objects.filter(unit_name=unit_name).order_by("-total_borrowed_books_num")[0]
