...
def get_service_url(request: HttpRequest, redirect_to: Optional[str] = None) -> str:
    ...
    # ----- MODIFIED START ----- #
    if hasattr(django_settings, 'CAS_SERVICE_URL'):
        return django_settings.CAS_SERVICE_URL
    # ----- MODIFIED END ----- #
...
def get_cas_client(
    ...
    service_validate_url = django_settings.CAS_SERVICE_VALIDATE_URL
    logout_url = django_settings.CAS_LOGOUT_URL
    proxy_url = django_settings.CAS_PROXY_URL
    # ----- MODIFIED END ----- #
    ...
    kwargs = dict(
        ...
        login_url=login_url,
        service_validate_url=service_validate_url,
